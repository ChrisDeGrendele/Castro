subroutine amrex_probinit(init, name, namlen, problo, probhi) bind(C, name="amrex_probinit")
  ...
  !!!!THESE MIGHT NEED TO BE CHANGED LATER??? CALCULATE USING P AND RHO?
  ...
  do k = lo(3), hi(3)
     do j = lo(2), hi(2)
        do i = lo(1), hi(1)
           ...
           if ((ABS(i-105)<3) .AND. (ABS(j-64)<3)) then
              state(i,j,k,URHO) = rho_bump
              state(i,j,k,UEINT) = e_bump
              state(i,j,k,UFS  ) = state(i,j,k,URHO)
           endif
...
subroutine ca_initrad(level, time, lo, hi, nrad, &
  ...
  use rad_params_module, only : xnu, pi, clight, hplanck, kboltz, stefbol
...
!  enddo
  ...
  !TAKEN FROM RADSOURCETEST LETS TRY THIS TO TRY TO REMOVE THE ARTIFICAL SHOCK COMING FROM BC
