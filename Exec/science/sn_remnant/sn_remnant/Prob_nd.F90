subroutine amrex_probinit (init,name,namlen,problo,probhi) bind(c)
  ...
  dens_ambient = 5.e-6_rt      ! ambient density (in g/cc)
  exp_energy = 1.e0_rt        ! absolute energy of the explosion (in erg)
  r_init = 2.e12_rt          ! initial radius of the explosion (in cm)
  nsub = 4
  temp_ambient = 1.e3_rt     ! Set original temp. to negative, which is overwritten in the probin file
              ...
              do jj = 0, nsuby-1
                 ...
                 do ii = 0, nsubx-1
                    ...
                    if (dist <= r_init**2) then
                       pert =  .true.
                       state(i,j,k,UTEMP) = 10**7
                    else
                       pert = .false.
                       state(i,j,k,UTEMP) = 10**3
                    end if
...
subroutine ca_initrad(level, time, lo, hi, nrad, &
  ...
  real(rt), intent(in) :: problo(3), probhi(3), time, dx(3)
  ...
  integer :: i, j, k, igroup, ii, jj,kk
  real(rt)         length_cell, T
  real(rt) :: xmin,ymin,zmin
  real(rt) :: xx, yy, zz, dist
  integer :: nsubx, nsuby, nsubz
  real(rt) :: ds(3)
